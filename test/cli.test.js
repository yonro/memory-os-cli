...
test('mcp list exposes supported clients without token values', async () => {
  ...
  assert.deepEqual(clients.map((client) => client.id), ['codex', 'cursor', 'gemini-cli', 'antigravity', 'antigravity-ide', 'antigravity2', 'antigravity-cli', 'windsurf', 'cline', 'continue', 'claude-desktop', 'openclaw', 'copilot-cli']);
  assert.doesNotMatch(result.stdout, /secret-token-that-must-not-leak/);
});
