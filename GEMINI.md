<!-- memory-os:memory-profile:gemini-cli:start -->
## XMemo Gemini CLI profile

MCP server: `XMemo`

Use XMemo deliberately through MCP for project context recall and high-signal write-back.

Recommended Gemini CLI behavior:
...
- Use the client-managed MCP OAuth credential; do not paste token values into prompts, config files, or logs.

<!-- memory-os:memory-profile:gemini-cli:end -->

<!-- memory-os:memory-profile:antigravity:start -->
## XMemo Antigravity profile
...
<!-- memory-os:memory-profile:antigravity:end -->
