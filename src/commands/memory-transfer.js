...
export async function prepareMemoryTransfer(command, args) {
  ...
  assertKnownOptions(args, [...common, '--limit', '--bucket', '--scope', '--file', '--dry-run', '--idempotency-key', '--yes']);
  const limit = number(args, '--limit', 500, 5000);
  const body = { limit, bucket: optionValue(args, '--bucket') ?? 'private' };
...
export async function memoryTransfer(command, args, io, context, prepared) {
  ...
  if (!body.dry_run) await confirmRemoteAction(args, io, 'Import these memories into the selected space?');
  const pages = [];
  let cursor = 0;
  do {
    const response = await context.client.request({ method: 'POST', path: '/v1/memories/import', body: { ...body, cursor }, sideEffect: !body.dry_run });
