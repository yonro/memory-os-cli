...
export function writeHelp(io) {
  ...
  writeLine(io.stdout, `  ${COMMAND_NAME} memory add|search ... [--json]`);
  writeLine(io.stdout, '      Add or search memory through the REST service.');
  writeLine(io.stdout, `  ${COMMAND_NAME} context recall <query> [--json]`);
  writeLine(io.stdout, '      Recall a bounded agent context package.');
  writeLine(io.stdout, `  ${COMMAND_NAME} state save|restore ... [--json]`);
  writeLine(io.stdout, '      Save or restore one working state slot.');
  writeLine(io.stdout, `  ${COMMAND_NAME} restart snapshot|restore ... [--json]`);
  writeLine(io.stdout, '      Create a restart package or explicitly restore its working state.');
  writeLine(io.stdout, `  ${COMMAND_NAME} knowledge add|search|read|update ... [--json]`);
  writeLine(io.stdout, '      Add, search, read, or safely update knowledge items.');
  writeLine(io.stdout, `  ${COMMAND_NAME} dream preview|show|apply ... [--json]`);
  writeLine(io.stdout, '      Preview, inspect, or confirm one Dream candidate.');
  writeLine(io.stdout, `  ${COMMAND_NAME} cloud-skill add|list|show|update|run ... [--json]`);
  writeLine(io.stdout, '      Inspect and execute Cloud Skills; safe writes require the server CAS contract.');
  writeLine(io.stdout, '');
